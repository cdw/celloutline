...
__all__ = ["PCA", "ICA", "Fourier", "Zernicke", "Autoencoder"]
...
class PCA(Model):
    ...
    def __init__(self, fn=None, n_comp=None, batch_size=None):
        ...
        self.fn = fn
        self.params = {"n_components": n_comp, 
...
class Fourier:
    ...
    def _mask(self):
        ...
        to_keep = int(round(
            np.divide(np.power(self.reduced_dimensions, 1/3), 2)))
        mask = np.zeros(self.original_shape)
        mask[center[0]-to_keep : center[0]+to_keep, 
             center[1]-to_keep : center[1]+to_keep,
             center[2]-to_keep : center[2]+to_keep] = 1
        return mask
    ...
    def encode(self, input):
        ...
        shiftedfftcell = np.fft.fftshift(fftcell) # shift max freq around center
